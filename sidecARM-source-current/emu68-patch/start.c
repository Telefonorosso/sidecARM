...
/*
 * PiStorm secondary-core service POC.
 * Control stays deliberately tiny (MOVEC); bulk data lives in the ZIII
 * armservice aperture and is loaded by AmigaOS from any filesystem.
 */
#if defined(PISTORM_ANY_MODEL)
volatile uint32_t linux_poc_cmd = 0;
volatile uint32_t linux_poc_status = 0;
volatile uint32_t linux_poc_data = 0;
volatile uint32_t linux_poc_entry_off = 0;
volatile uint32_t linux_poc_arg0_off = 0;

#define LINUX_POC_ST_READY    0x52445932U /* RDY2 */
#define LINUX_POC_ST_ALIVE    0x414c4956U /* ALIV */
#define LINUX_POC_ST_RUNNING  0x52554e32U /* RUN2 */
#define LINUX_POC_ST_PAYLOAD  0x50415932U /* PAY2 */
#define LINUX_POC_ST_NOBOARD  0x4e4f4244U /* NOBD */
#define LINUX_POC_ST_FLUSH    0x464c5348U /* FLSH */
#define LINUX_POC_ST_HANDOFF  0x484e444fU /* HNDO */
#define LINUX_POC_ST_LINUX    0x4c4e5833U /* LNX3 */

extern volatile uint32_t armservice_z3_base;
extern volatile uintptr_t armservice_exec_base;
extern uintptr_t armservice_phys_base;

#define ARMSERVICE_SIZE (256U * 1024U * 1024U)

/*
 * Generic EL1 physical handoff.
 *
 * This MUST NOT be a normal C function: after SCTLR_EL1.M is cleared even a
 * compiler-generated stack access would be fatal.  GCC AArch64 does not
 * support the "naked" attribute used by some other backends, so define the
 * tiny trampoline as a top-level assembler symbol.
 *
 * ABI on entry:
 *   x0 = physical Linux entry point
 *   x1 = physical DTB address
 *   x2 bit 0 = Linux Image endianness flag
 */
extern void armservice_physical_handoff(uintptr_t entry_phys,
                                        uintptr_t arg0_phys,
                                        uint64_t flags)
    __attribute__((noreturn));

__asm__(
    ".pushsection .text.armservice_handoff,\"ax\",%progbits\n"
    ".balign 64\n"
    ".global armservice_physical_handoff\n"
    ".type armservice_physical_handoff,%function\n"
    "armservice_physical_handoff:\n"
    "    mov x9,  x0\n"
    "    mov x10, x1\n"
    "    msr daifset, #0xf\n"
    "    dsb sy\n"
    ...
    "    mov x3, xzr\n"
    "    br x9\n"
    ".size armservice_physical_handoff, .-armservice_physical_handoff\n"
    ".popsection\n"
);

static __attribute__((noreturn)) void linux_poc_cpu1_service(void)
{
    uint64_t cpu_id;

    __asm__ volatile("mrs %0, MPIDR_EL1" : "=r"(cpu_id));
    linux_poc_data = (uint32_t)(cpu_id & 3U);
    linux_poc_status = LINUX_POC_ST_READY;
    __asm__ volatile("dsb sy; sev" ::: "memory");

    for (;;) {
        uint32_t cmd = __atomic_exchange_n(&linux_poc_cmd, 0, __ATOMIC_ACQ_REL);

        if (cmd == 1U) {
            linux_poc_data = (uint32_t)(cpu_id & 3U);
            linux_poc_status = LINUX_POC_ST_ALIVE;
            __asm__ volatile("dsb sy; sev" ::: "memory");
        }
        else if (cmd == 2U) {
            uint32_t rw_base = armservice_z3_base;
            uintptr_t exec_base = armservice_exec_base;
            if (!rw_base || !exec_base) {
                linux_poc_data = 0;
                linux_poc_status = LINUX_POC_ST_NOBOARD;
                __asm__ volatile("dsb sy; sev" ::: "memory");
            } else {
                uint32_t (*payload_entry)(void) =
                    (uint32_t (*)(void))exec_base;

                linux_poc_status = LINUX_POC_ST_RUNNING;
                linux_poc_data = 0;
                __asm__ volatile("dsb sy" ::: "memory");

                /*
                 * The Amiga writes through the ZIII RW alias.  CPU1 executes
                 * through a distinct RO alias of the same physical backing,
                 * matching Emu68's existing JIT W^X model.
                 */
                arm_flush_cache((uintptr_t)rw_base, 4096);
                arm_icache_invalidate(exec_base, 4096);
                __asm__ volatile("dsb sy; isb" ::: "memory");

                linux_poc_data = payload_entry();
                linux_poc_status = LINUX_POC_ST_PAYLOAD;
                __asm__ volatile("dsb sy; sev" ::: "memory");
            }
        }
        else if (cmd == 3U) {
            uintptr_t phys = armservice_phys_base;
            uint32_t entry_off = linux_poc_entry_off;
            uint32_t arg0_off = linux_poc_arg0_off;

            if (!armservice_z3_base || !phys ||
                entry_off >= ARMSERVICE_SIZE ||
                arg0_off >= ARMSERVICE_SIZE) {
                linux_poc_data = 0;
                linux_poc_status = LINUX_POC_ST_NOBOARD;
                __asm__ volatile("dsb sy; sev" ::: "memory");
            } else {
                uintptr_t entry_phys = phys + entry_off;
                uintptr_t arg0_phys = phys + arg0_off;
                uintptr_t handoff_va = (uintptr_t)armservice_physical_handoff;
                uintptr_t handoff_phys = mmu_virt2phys(handoff_va);
                uintptr_t handoff_page = handoff_phys & ~(uintptr_t)0xfffU;
                void (*handoff)(uintptr_t, uintptr_t, uint64_t);
                uint64_t image_flags;

                /*
                 * The Linux Image header lives at the entry itself.
                 * flags is a little-endian u64 at +24.  Emu68 is normally BE,
                 * so decode bytewise instead of dereferencing a u64.
                 */
                volatile const uint8_t *hdr =
                    (volatile const uint8_t *)(uintptr_t)
                    (armservice_z3_base + entry_off);
                image_flags =
                    ((uint64_t)hdr[24]      ) |
                    ((uint64_t)hdr[25] <<  8) |
                    ((uint64_t)hdr[26] << 16) |
                ...
                /*
                 * Expose the expensive cache-maintenance phase to AmigaOS.
                 * linux_poc_status is already readable through MOVEC 0x1e2.
                 */
                linux_poc_status = LINUX_POC_ST_FLUSH;
                __asm__ volatile("dsb sy; sev" ::: "memory");

                /*
                 * Commit all Amiga-written service RAM to PoC before Linux
                 * enters with the MMU off.
                 */
                arm_flush_cache((uintptr_t)armservice_z3_base,
                                ARMSERVICE_SIZE);

                linux_poc_status = LINUX_POC_ST_HANDOFF;
                __asm__ volatile("dsb sy; sev" ::: "memory");

                /*
                 * Execute the handoff itself at VA==PA.  The page is RO so
                 * Emu68's SCTLR.WXN policy still holds while MMU is enabled.
                 */
                mmu_map(handoff_page, handoff_page, 4096,
                        MMU_ACCESS | MMU_ISHARE |
                        MMU_READ_ONLY | MMU_ATTR_CACHED, 0);
                arm_icache_invalidate(handoff_page, 4096);
                __asm__ volatile("dsb sy; isb" ::: "memory");

                linux_poc_data = (uint32_t)entry_phys;
                linux_poc_status = LINUX_POC_ST_LINUX;
                __asm__ volatile("dsb sy; sev" ::: "memory");

                handoff = (void (*)(uintptr_t, uintptr_t, uint64_t))
                          handoff_phys;
                handoff(entry_phys, arg0_phys, image_flags);
