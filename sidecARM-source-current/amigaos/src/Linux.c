...
int main(int argc, char **argv)
{
    ...
    if (argc != 5) {
        printf("Usage: Linux <Image> <dtb> <initramfs.cpio> <diskfile>\n");
        printf("Example: Linux kernel8.img pinux.dtb initramfs.cpio Work:linux.img\n");
        return 5;
    }
    ...
    if (text_offset >= INITRD_OFF ||
        (unsigned long long)file_size > (unsigned long long)(INITRD_OFF - (ULONG)text_offset)) {
        printf("Image does not fit service RAM: offset=%08lx file=%ld\n",
    ...
    if (!load_at(argv[1], board + (ULONG)text_offset,
                 DTB_OFF - (ULONG)text_offset, NULL)) {
