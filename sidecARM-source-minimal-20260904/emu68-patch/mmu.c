    ...
    if (e)
    {
        ...
        for (int block=0; block < block_count; block++)
        {
            ...
            */
            ...
            /*
             * ARM service RAM is deliberately the LAST top-of-RAM carveout.
             *
             * Emu68 has already reserved MMU/JIT, VideoCore and Unicam space
             * above us.  Taking the service block here gives it a single,
             * exclusive physical owner and ensures the reduced /memory node
             * published to AmigaOS can never overlap it.
             */
            if (last_block) {
                extern uintptr_t armservice_phys_base;
                extern volatile uint32_t armservice_phys_base_68k;
                const uintptr_t armservice_size = 128U * 1024U * 1024U;

                if (size >= armservice_size) {
                    size -= armservice_size;
                    armservice_phys_base = addr + size;
                    armservice_phys_base_68k = (uint32_t)armservice_phys_base;
                    update_needed = 1;
                    kprintf("[ARMSVC] reserved physical %08lx-%08lx "
                            "(256 MiB, final top-of-RAM carveout)\n",
                            armservice_phys_base,
                            armservice_phys_base + armservice_size - 1);
                } else {
                    armservice_phys_base = 0;
                    armservice_phys_base_68k = 0;
                    kprintf("[ARMSVC] ERROR: cannot reserve 256 MiB "
                            "from final memory block\n");
                }
            }
